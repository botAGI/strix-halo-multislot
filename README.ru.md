...
Рецепты мультислот-инференса и честные бенчи на AMD Strix Halo
(Ryzen AI Max+ 395, Radeon 8060S, 128 ГБ общей памяти). Один мини-ПК держит 32
одновременных потока на **236 tok/s суммарно** (Gemma4 26B A4B, llama.cpp Vulkan/RADV),
выдерживает это 30 минут без троттлинга и разгоняет **~90 tok/s на одном потоке**
со встроенным MTP-драфтом. Всё воспроизводится на любом Strix Halo мини-ПК за вечер:
скрипты запуска в `recipes/`, харнессы в `bench/`, полные таблицы в `results/`.
