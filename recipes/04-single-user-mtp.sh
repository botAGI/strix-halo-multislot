...
#
# КОГДА ВКЛЮЧАТЬ MTP (замерено, acceptance 69-74% на смешанных задачах):
#   1 поток: +40%  |  2 потока: +21%  |  4 потока: −31%  |  32 потока: −33%
# Спекулятив выигрывает, пока decode упирается в память; на батче GPU уже
# compute-bound и драфт — налог. Правило: 1-2 юзера — ON, больше — OFF.
#
